...
// ── 거리 모드: 우선순위1(access_mode 정확 일치) + 우선순위2(직선거리 밴드) ──
test('reachableInMode 우선순위1: accessMode는 정확히 그 모드에서만 노출 (거리 무시)', () => {
  // access_mode=도보 → 도보만 노출, 따릉이·택시엔 절대 안 뜸 (핵심 요구사항)
  assert.equal(reachableInMode({ distanceM: 500, accessMode: 'walk' }, 'walk'), true);
  assert.equal(reachableInMode({ distanceM: 500, accessMode: 'walk' }, 'bike'), false);
  assert.equal(reachableInMode({ distanceM: 500, accessMode: 'walk' }, 'taxi'), false);
  // access_mode=택시 → 코앞(350m)이라도 도보·따릉이 제외, 택시만
  assert.equal(reachableInMode({ distanceM: 350, accessMode: 'taxi' }, 'walk'), false);
  assert.equal(reachableInMode({ distanceM: 350, accessMode: 'taxi' }, 'bike'), false);
  assert.equal(reachableInMode({ distanceM: 350, accessMode: 'taxi' }, 'taxi'), true);
  // 거리 완전 무시: 4km라도 도보 지정이면 도보에서 노출
  assert.equal(reachableInMode({ distanceM: 4000, accessMode: 'walk' }, 'walk'), true);
});
test('reachableInMode 우선순위2: accessMode 없으면 겹치지 않는 거리 밴드로 단 하나의 모드에만', () => {
  // 도보 0~700
  assert.equal(reachableInMode({ distanceM: 500 }, 'walk'), true);
  assert.equal(reachableInMode({ distanceM: 500 }, 'bike'), false);
  assert.equal(reachableInMode({ distanceM: 500 }, 'taxi'), false);
  // 따릉이 701~1500
  assert.equal(reachableInMode({ distanceM: 900 }, 'walk'), false);
  assert.equal(reachableInMode({ distanceM: 900 }, 'bike'), true);
  assert.equal(reachableInMode({ distanceM: 900 }, 'taxi'), false);
  // 택시 1501~3000
  assert.equal(reachableInMode({ distanceM: 2000 }, 'walk'), false);
  assert.equal(reachableInMode({ distanceM: 2000 }, 'bike'), false);
  assert.equal(reachableInMode({ distanceM: 2000 }, 'taxi'), true);
  // 경계값: 700=도보, 701=따릉이, 1500=따릉이, 1501=택시, 3000=택시, 3001=어느 모드도 아님
  assert.equal(reachableInMode({ distanceM: 700 }, 'walk'), true);
  assert.equal(reachableInMode({ distanceM: 701 }, 'walk'), false);
  assert.equal(reachableInMode({ distanceM: 701 }, 'bike'), true);
  assert.equal(reachableInMode({ distanceM: 1500 }, 'bike'), true);
  assert.equal(reachableInMode({ distanceM: 1501 }, 'bike'), false);
  assert.equal(reachableInMode({ distanceM: 1501 }, 'taxi'), true);
  assert.equal(reachableInMode({ distanceM: 3000 }, 'taxi'), true);
  assert.equal(reachableInMode({ distanceM: 3001 }, 'taxi'), false);
});
test('buildCandidates: 택시 지정 근처 식당은 도보·따릉이 제외·택시만 포함', async () => {
  // mock '언덕 위 감자탕' = 직선 ~358m지만 access_mode=taxi → 정확 일치라 택시에서만
  const walk = await buildCandidates(COMPANY_COORDS, 'lunch-group', 'walk');
  const bike = await buildCandidates(COMPANY_COORDS, 'lunch-group', 'bike');
  const taxi = await buildCandidates(COMPANY_COORDS, 'lunch-group', 'taxi');
  assert.equal(walk.candidates.some((c) => c.name === '언덕 위 감자탕'), false, '도보 제외');
  assert.equal(bike.candidates.some((c) => c.name === '언덕 위 감자탕'), false, '따릉이 제외');
  assert.equal(taxi.candidates.some((c) => c.name === '언덕 위 감자탕'), true, '택시만 노출');
});
test('buildCandidates: access_mode 없는 매장은 밴드에 맞는 모드에서만 노출', async () => {
  // 대치 정성백반 ~984m(따릉이 밴드), 크리스탈제이드 도곡점 ~156m(도보 밴드, solo_friendly=FALSE)
  const walk = await buildCandidates(COMPANY_COORDS, 'lunch-group', 'walk');
  const bike = await buildCandidates(COMPANY_COORDS, 'lunch-group', 'bike');
  const taxi = await buildCandidates(COMPANY_COORDS, 'lunch-group', 'taxi');
  // 대치 정성백반: 따릉이에만
  assert.equal(walk.candidates.some((c) => c.name === '대치 정성백반'), false);
  assert.equal(bike.candidates.some((c) => c.name === '대치 정성백반'), true);
  assert.equal(taxi.candidates.some((c) => c.name === '대치 정성백반'), false);
  // 크리스탈제이드(도곡): 도보에만
  assert.equal(walk.candidates.some((c) => c.name.includes('크리스탈제이드')), true);
  assert.equal(bike.candidates.some((c) => c.name.includes('크리스탈제이드')), false);
});
...
test('applyFilters: 거리 방어선 — 밴드 밖 후보는 모드별로 갈린다', () => {
  // 낡은 풀이 넘어온 상황 재현: 도보(0~700) 밴드 밖 후보는 도보에서 빠지고, 자기 밴드에서만 뜬다.
  const walkC = { ...cand('walkC', '샐러드·포케'), distanceM: 500 };  // 도보 밴드
  const bikeC = { ...cand('bikeC', '샐러드·포케'), distanceM: 900 };  // 따릉이 밴드
  const taxiC = { ...cand('taxiC', '샐러드·포케'), distanceM: 2000 }; // 택시 밴드
  const pool = [walkC, bikeC, taxiC];
  const pick = (distance: 'walk' | 'bike' | 'taxi') =>
    applyFilters(pool, { excludedSubs: [], priceTier: null, seenIds: [], distance }).map((c) => c.id);
  assert.deepEqual(pick('walk'), ['walkC']);
  assert.deepEqual(pick('bike'), ['bikeC']);
  assert.deepEqual(pick('taxi'), ['taxiC']);
});
test('applyFilters: access_mode=도보면 거리 무관 통과하되 도보에서만 (관리자 지정 우선)', () => {
  const far = { ...cand('far', '샐러드·포케'), distanceM: 2500, accessMode: 'walk' as const };
  const pick = (distance: 'walk' | 'bike' | 'taxi') =>
    applyFilters([far], { excludedSubs: [], priceTier: null, seenIds: [], distance }).map((c) => c.id);
  assert.deepEqual(pick('walk'), ['far'], '멀어도 도보 지정이면 도보에서 노출');
  assert.deepEqual(pick('bike'), [], '따릉이엔 절대 안 뜸');
  assert.deepEqual(pick('taxi'), [], '택시엔 절대 안 뜸');
});
...
test('buildCandidates 혼밥: solo_friendly=FALSE 매장은 카카오 트윈으로도 재등장 안 함', async () => {
  ...
  // (도곡점 ~156m라 도보 밴드에서 검증한다.)
  const solo = await buildCandidates(COMPANY_COORDS, 'lunch-solo', 'walk');
  ...
  const group = await buildCandidates(COMPANY_COORDS, 'lunch-group', 'walk');
  assert.ok(
...
// ── 구글 품질 게이트 (§12) ──
const GATE = { minRating: 4.0, minReviews: 200, minRatingReviews: 10, badRating: 3.0 };
test('passesGate: 평점 4.0↑ + 리뷰 10↑, 또는 리뷰 200↑', () => {
  // 평점 좋고 리뷰 하한 충족 → 통과
  assert.equal(passesGate({ rating: 4.2, reviews: 15 }, GATE), true);
  // 리뷰만 많아도 통과 (평점이 3점 초과이면서 — 지뢰 컷 위)
  assert.equal(passesGate({ rating: 3.1, reviews: 500 }, GATE), true);
  // 리뷰 200 경계 포함
  assert.equal(passesGate({ rating: 3.5, reviews: 200 }, GATE), true);
  // 둘 다 미달이면 탈락
  assert.equal(passesGate({ rating: 3.9, reviews: 199 }, GATE), false);
});
test('passesGate: 평점 통과에 최소 리뷰 하한 — 리뷰 1~2개짜리 ★5 컷', () => {
  // 표본이 없는 고평점 (사장님 지인일 수 있음)
  assert.equal(passesGate({ rating: 5.0, reviews: 1 }, GATE), false);
  assert.equal(passesGate({ rating: 4.7, reviews: 3 }, GATE), false);
  assert.equal(passesGate({ rating: 4.0, reviews: 9 }, GATE), false); // 하한 바로 아래
  // 리뷰 10개 경계는 통과
  assert.equal(passesGate({ rating: 4.0, reviews: 10 }, GATE), true);
});
test('passesGate: 지뢰 컷 — 평점 3.0 이하는 리뷰 많아도 탈락', () => {
  // 핵심: 리뷰가 많다는 건 검증됐다는 뜻인데도 평점이 낮으면 '피해야 할 곳'
  assert.equal(passesGate({ rating: 2.8, reviews: 999 }, GATE), false);
  assert.equal(passesGate({ rating: 3.0, reviews: 500 }, GATE), false); // 3.0 '이하' 포함
  // 3.0 바로 위는 리뷰 기준으로 통과
  assert.equal(passesGate({ rating: 3.1, reviews: 200 }, GATE), true);
  // 리뷰가 적어도 저평점이면 당연히 탈락
  assert.equal(passesGate({ rating: 1.0, reviews: 5 }, GATE), false);
});
