...
> 중복 제거(이름+50m)는 두 경우 동일. 한 번의 `/api/sync` 호출로 둘 다 실행.
...
```
name	category_sub	signature_menu	price_note	address	lat	lng	comment	active	weight	phone	visited	recommended
```
...
- 이미 있는 매장은 이름+50m 중복 판정으로 제외 → 중복 안 쌓임. 매일 돌면서 신규만 축적.
