...
- `src/lib/categories.ts` 2단계 카테고리 31종(일식 장어요리, 한식 칼국수/냉면·갈비탕/샤브샤브, 아시안 홍콩·딤섬, 양식 멕시칸·타코 포함) + 카카오 매핑(세부→대분류→기타. **규칙은 배열 순서대로 첫 매치 채택 — 샤브샤브가 칼국수보다 위(등촌샤브칼국수), 냉면·갈비탕이 국밥·탕('탕')·고기구이('갈비')보다 위, 일반 '국수'는 칼국수·쌀국수보다 아래여야 함**)
...
- **카테고리 도트 아이콘 (식사 30종 + 후식 5종 coffee/bakery/cake/donut/icecream + fallback)** — `scripts/gen-icons.mjs`(문자 그리드→PNG, 재실행 가능), `public/assets/icons/*.png`, 매핑 `src/lib/icons.ts`, 렌더 `src/components/DotIcon.tsx`.
...
4. 거리 판정 = **직선거리(하버사인)** 기준. '도보 예상 분' 표시에만 ×1.3 보정. 길찾기 API 미사용. 판정은 `geo.ts` `reachableInMode` 한 곳(서버 `buildCandidates` + 프론트 `applyFilters` 공용). **두 우선순위(v1.17):**
   - **우선순위1 — `access_mode`(시트 T열, 1=도보/2=따릉이/3=택시) 값이 있으면 정확히 그 모드에서만 노출**(직선거리 무시). 예) `1`(도보)이면 따릉이·택시 선택 시 **절대 안 뜸.** curated 후보에만 적용(카카오는 값 없음). **1/2/3 외 값은 조용히 무시되고 밴드 판정으로 처리**되니 오타 주의.
   - **우선순위2 — `access_mode` 없으면 직선거리로 겹치지 않는 밴드에 배정** → 각 매장은 단 하나의 모드에만 노출: **도보 0~700m / 따릉이 701~1500m / 택시 1501~3000m** (3000m 초과는 어느 모드에도 미노출). 카카오 반경 검색도 밴드 최댓값(700/1500/3000)까지만.
   - v1.15의 `accessModeWeight`·`distancePrefWeight`(반경 상대 거리 가중치)는 **폐지** — 밴드가 겹치지 않아 모드별 풀이 이미 갈리므로 불필요.
