...
**점심은 위치인식 미사용 — 시작점 항상 군인공제회관 고정** (`COMPANY_COORDS`). **후식만 예외: 브라우저 현재 위치 반경 500m** (거부 시 군인공제회관 폴백).
...
- `dessert` 후식: **하위 분기 없음**. 현재 위치 반경 500m(부족 시 1km 자동 확장). 카카오 CE7(카페) + `coffee` 시트. 예산·거리 필터 없음, 제외 메뉴(후식 5종)만. `recommended` 우선 부스트.
...
- `src/lib/categories.ts` 2단계 카테고리 25종(일식 장어요리 포함) + 카카오 매핑(세부→대분류→기타) + 예산추정 + 혼밥휴리스틱 + **후식 5종(`DESSERT_SUBS`)·`mapKakaoCafe`(CE7)**
...
- `src/lib/candidates.ts` ★ 점심 `buildCandidates` + **후식 `buildDessertCandidates`(500m·자동확장)** + 중복병합(이름+50m, DB우선) + 가중치
...
- **카테고리 도트 아이콘 (식사 24종 + 후식 5종 coffee/bakery/cake/donut/icecream + fallback)** — `scripts/gen-icons.mjs`(문자 그리드→PNG, 재실행 가능), `public/assets/icons/*.png`, 매핑 `src/lib/icons.ts`, 렌더 `src/components/DotIcon.tsx`.
...
3. 점심은 위치인식 미사용(시작점 군인공제회관 고정). **후식만 예외 — 브라우저 현재 위치 반경 500m, 거부 시 군인공제회관 폴백.**
