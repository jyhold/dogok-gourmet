...
const VALID_MODES: Mode[] = ['lunch-solo', 'lunch-group', 'dinner-flash', 'dinner-team'];
...
export async function GET(req: NextRequest) {
  ...
  // 사내용: 시작점은 항상 군인공제회관 고정 (위치인식 미사용)
  const center = COMPANY_COORDS;

  try {
    const { candidates, usedFallback } = await buildCandidates(center, mode, distance);
