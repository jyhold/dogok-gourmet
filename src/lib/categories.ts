...
export const CATEGORY_TREE: CategoryDef[] = [
  ...
  { main: '양식', subs: ['파스타·피자', '버거', '스테이크·비스트로', '멕시칸·타코'] },
  { main: '아시안', subs: ['쌀국수·베트남', '태국·팟타이', '인도·커리', '홍콩·딤섬'] },
...
const SUB_RULES: MappingRule[] = [
  ...
  // 홍콩·딤섬 — 카카오가 '중식 > 딤섬,만두'/'중식 > 홍콩요리'로 주므로 아래 만두('만두')·
  // 중식('중식') 규칙보다 반드시 위여야 그쪽에 뺏기지 않는다. '홍콩'은 홍콩반점류(중식 > 중국집,
  // category_name에 '홍콩' 없음)와 겹치지 않게 '홍콩요리'로만 잡는다.
  { keywords: ['딤섬', '홍콩요리', '완탕'], result: { main: '아시안', sub: '홍콩·딤섬' } },
  ...
  // 멕시칸 — 카카오는 '양식 > 멕시칸,브라질'로 준다(타코 전문점 도스타코스·더타코부스 포함).
  // '타코'는 '타코야끼'(간식)에도 걸리므로 키워드에서 제외 — '멕시칸'만으로 다 잡힌다.
  { keywords: ['멕시칸', '멕시코', '부리또', '케사디야', '퀘사디아'], result: { main: '양식', sub: '멕시칸·타코' } },
  // 아시안
